...
public:
  ...
  TEST_CASE(list_init)
  {
    ...
    if (x.size() == 2 and x.at(0) == std::byte{0x01} and
        x.at(1) == std::byte{0x02}) {
      return pass(std::source_location::current().function_name());
    }
    return fail(std::source_location::current().function_name());
  } catch (...) {
  ...
  TEST_CASE(vector_init_move)
  {
    ...
    CBORBstr x{std::move(y)};
    if (y.size() == 0 and x.size() == 2 and x.at(0) == std::byte{0x01} and
        x.at(1) == std::byte{0x02}) {
      return pass(std::source_location::current().function_name());
    }
    return fail(std::source_location::current().function_name());
  ...
  TEST_CASE(vector_init_copy)
  {
    ...
    CBORBstr x{y};
    if (y.size() == 2 and x.size() == 2 and x.at(0) == std::byte{0x01} and
        x.at(1) == std::byte{0x02}) {
      return pass(std::source_location::current().function_name());
    }
    return fail(std::source_location::current().function_name());
